...
import mgtrf.Tools.ArrayIndexComparator;
...
public class RunLDA {
	...
	static String path = "C:\\Exp\\lda\\20news\\";
	static int num_topics = 3;  //topic numbers
	static int VAR_MAX_ITER = 20;
	static double VAR_CONVERGED = 1e-6;
	static int EM_MAX_ITER = 100;
	static double EM_CONVERGED = 1e-4;
	static double alpha = 1;
	static double lambda2 = 0.6;
	static double lambda4 = 0.8;
	
	public static Corpus corpus;
	
	public static String perplexity = "";
...
//		    System.out.println(converged);
	...
	public static void run_em(Corpus corpus, int K, double l2, double l4)
	{
		num_topics = K;
		lambda2 = l2;
		lambda4 = l4;
		...
		model.save_lda_model(path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\", "init");		
		...
		while(((converged < 0) || (converged > EM_CONVERGED) || (i <= 2)) && (i <= EM_MAX_ITER))
		{
	        ...
	        model.save_lda_model(path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\model\\", i + "");
	        save_gamma(corpus, model, path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\model\\" + i + "_gamma");
	        save_doc_para(corpus, path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\model\\" + i + "_doc");
	        
		}		
		File likelihood_file = new File(path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\likelihood");
		try {
		...
		model.save_lda_model(path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\", "final");
		save_gamma(corpus, model, path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\final_gamma");
		save_doc_para(corpus, path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\final_doc");
		...
		for(int d = 0; d < corpus.num_docs; d++)
		{
			...
			save_word_assignment(corpus.docs[d], path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\word_topic_post\\" + corpus.docs[d].doc_name);
			save_top_words(10, corpus.docs[d], path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\top_word\\" + corpus.docs[d].doc_name);
		}
...
//			sb.append("\n");
		...
		perplexity += num_topics + " " + lambda2 + " " + lambda4 + " " + perplex + "\n";
		try {
			FileUtils.writeStringToFile(new File(path + "res_" + num_topics + "_" + lambda2 + "_" + lambda4 + "\\eval"), sb.toString());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
